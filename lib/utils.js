...
export const sendSMS = async (message, phoneNumbers) => {
  try {
    const options = {
      ...
      body: JSON.stringify({
        to: phoneNumbers,
        from: process.env.TERMII_SENDER_ID ?? "tlccglobal",
        sms: message,
        type: process.env.TERMII_SMS_TYPE ?? "plain",
        api_key:
          process.env.TERMII_SMS_TYPE ??
          "TLguPmKYUlYRArPklYzMorngEQKNwKPsorVfVdAlDYCHniZHeBrketfcsLpqLo",
        channel: process.env.TERMII_SMS_CHANNEL ?? "dnd",
      }),
    };

    fetch("https://api.sendchamp.com/api/v1/sms/send", options)
      .then((res) => res.json())
